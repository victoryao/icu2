...
public class XianhuoShoppingListItem extends BaseListItem<CartListNode> {

	private ImageView mImage;
	private TextView mTtile;
	private TextView mSn; // sn
	private TextView mTextCenter;
	private ImageView mArrow;
	private ImageView mShowType;
	private View mContainer;
	private View mTopLine;
	private static String TAG = "XianhuoShoppingListItem";

	public XianhuoShoppingListItem(Context context) {
		super(context, null);
	}

	public XianhuoShoppingListItem(Context context, AttributeSet attrs) {
		super(context, attrs);
		LayoutInflater.from(context).inflate(
				R.layout.xianhuo_shopping_cartlist, this, true);
	...
	public void bind(CartListNode data) {
		...
		mSn.setText("sn:" + data.getItemIds());
		mTextCenter.setText(String.format(
				getResources().getString(
						R.string.shopping_cartlist_text_center_template),
				data.getPrice(), data.getCount(), data.getTotal()));
		ImageLoader.getInstance().loadImage(mImage, data.getThumbnail(),
		...
		if (TextUtils.equals(showType, Tags.ShoppingCartList.SHOWTYPE_BARGIN)) {
			mTtile.setText(String.format(
					getResources().getString(
							R.string.cartitem_title_bargin_template),
					data.getTitle()));
		} else if (TextUtils.equals(showType,
				Tags.ShoppingCartList.SHOWTYPE_GIFT)) {
			mShowType.setImageResource(R.drawable.cartlist_item_showtype_gift);
		} else if (TextUtils.equals(showType,
				Tags.ShoppingCartList.SHOWTYPE_SECKILL)) {
			mShowType
					.setImageResource(R.drawable.cartlist_item_showtype_seckill);
		} else if (TextUtils.equals(showType,
				Tags.ShoppingCartList.SHOWTYPE_SPECIAL)) {
			mShowType
					.setImageResource(R.drawable.cartlist_item_showtype_special);
		} else if (TextUtils.equals(showType,
				Tags.ShoppingCartList.SHOWTYPE_ERNIE)) {
