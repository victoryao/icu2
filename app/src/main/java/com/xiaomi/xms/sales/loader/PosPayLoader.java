...
public class PosPayLoader extends BaseLoader<PosPayLoader.Result> {

	private final String CACHE_KEY = "PosPayLoader";
	private String serviceNumber;
	private String info;

	public PosPayLoader(Context context, String serviceNumber, String info) {
		super(context);
	...
	protected void initTaskList(List<AsyncTask<Void, Void, Result>> tasks) {
		tasks.add(new PosPayUpdateTask(false));
	}
	...
	private class PosPayUpdateTask extends BaseLoader<Result>.UpdateTask {
		public PosPayUpdateTask(boolean needSaveToDb) {
			super(needSaveToDb);
		}
		...
		protected Request getRequest() {
			...
			try {
				...
				String data = JsonUtil.creatRequestJson(
						HostManager.Method.METHOD_SAVEPAYINFO, jsonObject);
