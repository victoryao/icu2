...
public class ExchangeReplaceLoader extends BaseLoader<ExchangeReplaceLoader.Result> {

	private final String CACHE_KEY = "ExchangeReplaceLoader";
	private JSONArray replaceSn;


	public ExchangeReplaceLoader(Context context, JSONArray replaceSn) {
		super(context);
	...
	protected String getCacheKey() {
		return CACHE_KEY + replaceSn;
	}

	@Override
	protected void initTaskList(List<AsyncTask<Void, Void, Result>> tasks) {
		tasks.add(new ExchangeReplaceUpdateTask(false));
	}


	private class ExchangeReplaceUpdateTask extends BaseLoader<Result>.UpdateTask {
		public ExchangeReplaceUpdateTask(boolean needSaveToDb) {
			super(needSaveToDb);
		}
		...
		protected Request getRequest() {
			...
			try {
				...
				jsonObject.put(Tags.XMSAPI.KEYWORDS, Utils.Preference.getStringPref(getContext(), Constants.SameDayReturn.PREF_KEY_RETURN_ORDER_ID, ""));
				jsonObject.put(Tags.XMSAPI.IMEI, Device.IMEI);
				jsonObject.put(Tags.XMSAPI.OPTYPE, "REPLACE");
				jsonObject.put(Tags.XMSAPI.REPLACESN, replaceSn);
