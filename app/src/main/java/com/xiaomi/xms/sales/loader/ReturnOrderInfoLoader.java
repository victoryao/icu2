...
public class ReturnOrderInfoLoader extends BaseLoader<ReturnOrderInfoLoader.Result> {
	...
	public ReturnOrderInfoLoader(Context context) {
		super(context);
	...
	private class OrderInfoUpdateTask extends UpdateTask {
		...
		protected Request getRequest() {
			...
			try {
				...
				String data = JsonUtil.creatRequestJson(HostManager.Method.METHOD_DAYREFUNDREQUEST, jsonObject);
	...
	protected Result parseResult(JSONObject json, Result result) throws Exception {
		...
		if (result.mOrderInfo == null) {
			...
			Utils.Preference.setStringPref(getContext(), Constants.SameDayReturn.PREF_KEY_MERCHANT_NAME, result.mOrderInfo.getmMerchantName());
			Utils.Preference.setStringPref(getContext(), Constants.SameDayReturn.PREF_KEY_RETURN_ORDER_ID, result.mOrderInfo.getOrderId());
			Utils.Preference.setStringPref(getContext(), Constants.SameDayReturn.PREF_KEY_ORDER_DATE, result.mOrderInfo.getAddTime());
			Utils.Preference.setStringPref(getContext(), Constants.SameDayReturn.PREF_KEY_ORDER_DEVICE_NAME, result.mOrderInfo.getmPosName());
			...
			Utils.Preference.setStringPref(getContext(), Constants.SameDayReturn.PREF_KEY_REFERENCE_NUMBER, result.mOrderInfo.getReferenceNumber());
			Utils.Preference.setStringPref(getContext(), Constants.SameDayReturn.PREF_KEY_POSREQUSTID, String.valueOf(result.mOrderInfo.getPosRequstId()));
			Utils.Preference.setStringPref(getContext(), Constants.SameDayReturn.PREF_KEY_SERVICENUMBER, String.valueOf(result.mOrderInfo.getOrderId()));
		}
		return result;
	}
